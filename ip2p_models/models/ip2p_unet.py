...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        t_emb = self.time_proj(timesteps) # (batch, feature_dim)

        # timesteps does not contain any weights and will always return f32 tensors
        # but time_embedding might actually be running in fp16. so we need to cast here.
        # there might be better ways to encapsulate this.
        t_emb = t_emb.to(dtype=self.dtype)
        emb = self.time_embedding(t_emb) # (batch, feature_time_dim)
        ...
        if self.class_embedding is not None:
            ...
            class_emb = self.class_embedding(class_labels).to(dtype=self.dtype) # (batch, feature_class_dim)
            emb = emb + class_emb 

        # pre-process
        sample = self.conv_in(sample) # (batch, feature_dim, f, height, width)
