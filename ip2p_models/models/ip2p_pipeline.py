    ...
    def __call__(
        ...
        **kwargs,
        ...
        image_latents = self.prepare_image_latents(
            ...
            do_classifier_free_guidance,
            generator,
        ) # (batch*sequence_length, 4, h//4, w//4)
        image_latents = rearrange(image_latents, "(b f) c h w -> b c f h w", f=sequence_length)
        uncond_image_latents = torch.zeros_like(image_latents)
        image_latents = torch.cat([image_latents, image_latents, uncond_image_latents], dim=0) # [3b, 4,sequence_length, h//4, w//4]
        ...
        for i, t in enumerate(timesteps):
            ...
            scaled_latent_model_input = torch.cat([scaled_latent_model_input, image_latents], dim=1) # [3b, 8, sequence_length, h//4, w//4]
        ...
        image = self.vae.decode(latents.half(), return_dict=False)[0]
        image = rearrange(image, "(b f) c h w -> b c f h w", f=video_length)
        
        video = self.image_processor.denormalize(image)
        
        return InstructPix2PixOutput(videos=video)
